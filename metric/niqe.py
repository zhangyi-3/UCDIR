...
def estimate_aggd_param(block):
    ...
    rhat = (np.mean(np.abs(block)))**2 / (np.mean(block**2) + 1e-10)
...
def niqe(img,
    ...
    # crop image
    h, w = img.shape
    if block_size_h > min(h, w):
        print('reset niqe block size', min(h, w))
        block_size_h = min(h, w, block_size_h)
        block_size_w = min(h, w, block_size_w)
...
def calculate_niqe(img, crop_border, input_order='HWC', convert_to='y'):
    ...
    # we use the official params estimated from the pristine dataset.
    niqe_pris_params = np.load('./metric/niqe_pris_params.npz')
